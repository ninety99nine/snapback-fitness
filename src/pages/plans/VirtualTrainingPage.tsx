...
const GOALS = ["Lose weight", "Tone up", "Build strength", "Improve endurance", "Boost energy", "Postpartum recovery"];
...
const init: FormData = {
  name: "", isPostpartum: "", fitnessLevel: "",
  goals: [], healthNotes: "", scheduleDays: [], timePreference: "",
};
...
export default function VirtualTrainingPage() {
  ...
  const toggleArr = (key: "goals" | "scheduleDays", val: string) =>
    setForm(f => ({ ...f, [key]: f[key].includes(val) ? f[key].filter(v => v !== val) : [...f[key], val] }));

  const canNext = () => {
    if (step === 1) return !!form.name.trim() && !!form.isPostpartum && !!form.fitnessLevel;
    if (step === 2) return form.goals.length > 0;
    if (step === 3) return form.scheduleDays.length > 0 && !!form.timePreference;
    return true;
  };
  ...
  const waURL = () => {
    const msg = `Hi Patlo! 👋 I'm ready to start Virtual Training with SnapBack Fitness!

*Name:* ${form.name}
*Postpartum:* ${form.isPostpartum}
*Fitness Level:* ${form.fitnessLevel}
*Goals:* ${form.goals.join(", ")}${form.healthNotes ? `\n*Health Notes:* ${form.healthNotes}` : ""}
*Training Days:* ${form.scheduleDays.join(", ")}
*Preferred Time:* ${form.timePreference}

Plan: Virtual Training — P800 first month 💪`;
      ...
      <section className="bg-dark pt-28 pb-14 lg:pt-32 lg:pb-18">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <p className="font-body text-xs font-medium tracking-wide-custom text-secondary uppercase mb-3">PLAN 01</p>
            <h1 className="font-display text-h1 text-white uppercase mb-4 leading-none">Virtual Training</h1>
            <p className="font-body text-base text-white/65 max-w-lg mb-8">Full coaching from anywhere in Botswana. Patlo builds your personalised plan, tracks your progress, and supports you every step of the way.</p>
            <div className="flex flex-col gap-2">
              <div className="flex gap-2.5">
                <span className="flex items-center gap-1.5 bg-secondary/20 text-secondary px-4 py-1.5 rounded-full text-sm font-body font-semibold">
                  P800 first month
                </span>
                {[
                  { icon: Wifi, label: "Train from anywhere" },
                  { icon: Calendar, label: "Custom weekly schedule" },
                ].map(({ icon: Icon, label }) => (
            ...
            {!isDone ? (
                  ...
                  <motion.div key={step} custom={dir} variants={variants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.22, ease: "easeOut" }}>
                    ...
                    {step === 2 && (
                      <div className="space-y-6">
                        ...
                        <div>
                          <label className="block font-body text-sm font-medium text-[#0D0D0D]/65 mb-2">Select your goals — choose all that apply *</label>
                          <div className="flex gap-2 flex-wrap">
                            {GOALS.map(g => (
                              <Chip key={g} label={g} selected={form.goals.includes(g)} onClick={() => toggleArr("goals", g)} />
                            ))}
                          </div>
                        </div>
                        ...
                        <div>
                          <label className="block font-body text-sm font-medium text-[#0D0D0D]/65 mb-2">
                            Any injuries or health conditions? <span className="text-[#0D0D0D]/35 font-normal">optional</span>
                          </label>
                          <textarea
                            value={form.healthNotes}
                            onChange={e => setForm(f => ({ ...f, healthNotes: e.target.value }))}
                            placeholder="e.g. lower back pain, recent C-section, knee injury…"
                            rows={3}
                    ...
                    {step === 3 && (
                      <div className="space-y-6">
                        <h2 className="font-display text-2xl text-[#0D0D0D]">Build your weekly schedule</h2>

                        <div>
                          <label className="block font-body text-sm font-medium text-[#0D0D0D]/65 mb-3">Which days can you train? *</label>
                          <div className="grid grid-cols-7 gap-1.5">
                            {DAYS.map(d => (
                              <button
                                key={d}
                                type="button"
                                onClick={() => toggleArr("scheduleDays", d)}
                          ...
                          {form.scheduleDays.length > 0 && (
                            <p className="font-body text-xs text-secondary mt-2">{form.scheduleDays.length} day{form.scheduleDays.length > 1 ? "s" : ""} selected</p>
                          )}
                        </div>
                              ...
                              <button
                                ...
                                className={`w-full text-left px-4 py-3 rounded-xl border text-sm font-body transition-all duration-200 ${
                                  form.timePreference === t
                                    ...
                                    : "border-[#0D0D0D]/10 text-[#0D0D0D]/60 hover:border-secondary/35 hover:text-[#0D0D0D]/80"
                                }`}
                              >
                                {t}
                              </button>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                <div className="text-center mb-8">
                  ...
                  <p className="font-body text-sm text-[#0D0D0D]/55 max-w-xs mx-auto">Patlo will use these details to build your personalised virtual training plan.</p>
                </div>
                ...
                <div className="bg-[#F8F5FF] rounded-xl p-5 space-y-3 mb-6">
                  ...
                  {form.healthNotes && <SummaryRow label="Health Notes" value={form.healthNotes} />}
                  <SummaryRow label="Training Days" value={form.scheduleDays.join(", ")} />
                  <SummaryRow label="Preferred Time" value={form.timePreference} />
                  <div className="pt-2 border-t border-[#0D0D0D]/8">
                    <SummaryRow label="Plan" value="Virtual Training — P800 first month" highlight />
                  </div>
                </div>
